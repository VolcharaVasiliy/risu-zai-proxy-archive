...
- Нативное зрение DeepSeek через `deepseek-vision`: загрузка/обработка изображений, диагностика и лимит размера
...
Если Arena уже открыта и вы вошли в аккаунт, bridge можно подключить к этому
браузеру через CDP: запустите Edge/Chrome с `--remote-debugging-port=9222` и
задайте `LM_ARENA_BROWSER_CDP_URL=http://127.0.0.1:9222`. В этом режиме новые
временные профили и окна не создаются, а ваш браузер не закрывается. Bridge
больше не открывает временный браузер автоматически; такой режим включается
только явно через `LM_ARENA_ALLOW_TEMP_BROWSER=1`.
