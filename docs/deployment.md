...
| Multimodal image descriptions | optional `MULTIMODAL_IMAGE_MODE`, `MULTIMODAL_MAX_IMAGES`, `MULTIMODAL_CAPTION_MODEL`, `MULTIMODAL_CAPTION_PROMPT`, `MULTIMODAL_CAPTION_MAX_OUTPUT_TOKENS`, `DEEPSEEK_MAX_IMAGE_BYTES` |
...
| Mimo | `MIMO_SERVICE_TOKEN`, `MIMO_USER_ID`, `MIMO_PH_TOKEN`, `MIMO_COOKIE`, `MIMO_RESOLVE_IPS` (override China edge IP; proxy auto-resolves via China DoH by default — Smart DNS equivalent), `MIMO_DNS` (China DoH resolver, default AliDNS), `MIMO_PROXY` (China egress, alternative if source-IP guard remains), `MIMO_SKIP_TLS_VERIFY`, `MIMO_IMPERSONATE` |
| Kimi | `KIMI_TOKEN` |
