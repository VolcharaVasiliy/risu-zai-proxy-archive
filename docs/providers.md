...
| OpenCode Zen | `hy3-free`, `big-pickle`, `deepseek-v4-flash-free`, `mimo-v2.5-free`, `laguna-s-2.1-free`, `nemotron-3-ultra-free`, `nemotron-3.5-lightning-free` | — (none) | — | None (keyless OpenCode Zen free gateway) | — | Codex-specific provider. Wraps the OpenCode Zen `/zen/v1/chat/completions` upstream (auth stripped, browser User-Agent required). Exposes the same models Codex itself uses, with `reasoning_effort` passthrough and OpenAI tool schemas. No credentials needed — just pick any slug. Uses the prompt-tool shim for `tool_calls` the same way other chat-only providers do. |
...
| DeepSeek | `deepseek-chat`, `deepseek-reasoner`, `deepseek-search`, `deepseek-vision` | `DEEPSEEK_TOKEN` | `x-deepseek-token` header | Logged-in `chat.deepseek.com` session | `scripts/get-provider-creds.py` | Browser-session style token provider. `deepseek-vision` accepts OpenAI `image_url`/Responses `input_image`, uploads data or HTTPS images to DeepSeek, waits for processing, forks them to the vision file type, and sends them as `ref_file_ids`. Image requests disable search/thinking because the upstream vision flow is a separate model type. `DEEPSEEK_MAX_IMAGE_BYTES` defaults to 20 MiB. |
...
| LM Arena | Current bundled `arena.ai/text/direct` catalog (998 aliases / 966 scraped entries; e.g. `gpt-5.2`, `gemini-3-pro`, `qwen3-max`), exposed via `py/lmarena_models.json` | `LM_ARENA_COOKIE` or `LM_ARENA_STORAGE` | `LM_ARENA_HEADERS` (captured request headers) | Logged-in `arena.ai` browser session | Credentials Exporter scans cookies plus localStorage/sessionStorage/selected IndexedDB records and request headers | **Local-only**: each `POST /nextjs-api/stream/create-evaluation` needs a Google reCAPTCHA Enterprise v3 token. The token is score-gated and IP/fingerprint-bound to the solving browser, so the grabber and proxy must share the egress IP. |


## OpenCode Zen (Codex models)

`py/zen_proxy.py` is a Codex-specific provider that wraps the OpenCode Zen free gateway (`opencode.ai/zen/v1/chat/completions`). It mirrors the way the `codex-zen` project talks to Zen: the request auth is stripped, a browser `User-Agent` is required (the upstream returns `403` to the default `python-requests` UA), and `reasoning_effort` (`low`/`medium`/`high`/`xhigh`/`max`/`minimal`) is passed straight through.

- **No credentials.** Unlike most providers here, OpenCode Zen does not need cookies or API keys — it is the same keyless endpoint Codex itself uses.
- **Models** are the OpenCode Zen catalog slugs: `hy3-free`, `big-pickle`, `deepseek-v4-flash-free`, `mimo-v2.5-free`, `laguna-s-2.1-free`, `nemotron-3-ultra-free`, `nemotron-3.5-lightning-free`.
- **Tools.** OpenAI `tools`/`tool_choice`/`parallel_tool_calls` are forwarded; when the upstream cannot honor them natively, the global prompt-tool shim (`AGENT_TOOL_MODE=auto`) converts them, exactly like other chat-only providers.
- **Responses API.** Fully supported — `/v1/responses` routes Zen through the same `complete_response` / `stream_response_events` path as every other provider.
- **Aliases.** `max_output_tokens` → `max_tokens` and `reasoning.effort` → `reasoning_effort` are normalized, so Responses-API clients work unchanged.
- **Env knobs:** `ZEN_HOST` (default `opencode.ai`), `ZEN_BASE` (default `/zen/v1`), `ZEN_TIMEOUT` (default `120`).
...
1. **Existing browser CDP (recommended when already logged in).** Start your logged-in Edge/Chrome with remote debugging, set `LM_ARENA_BROWSER_CDP_URL` (for example `http://127.0.0.1:9222`), and run the bridge. It attaches to the existing `arena.ai` tab, does not create a temporary profile, and never closes your browser. The local endpoint remains `http://127.0.0.1:8772/mint`.
2. **Explicit temporary-browser fallback.** `scripts/lmarena-recaptcha-bridge.mjs` and `scripts/fetch-lmarena-recaptcha.mjs` launch a temporary browser only when `LM_ARENA_ALLOW_TEMP_BROWSER=1` is set. Their owned process tree is terminated and the temporary profile is removed on shutdown.
...
**Extension / manual integration.** Credentials Exporter writes `LM_ARENA_COOKIE`, a bounded `LM_ARENA_STORAGE` snapshot (localStorage/sessionStorage/selected auth-like IndexedDB records), and captured `LM_ARENA_HEADERS`. Any external process may also write `lmarena-recaptcha.json` (or `LM_ARENA_CAPTCHA_FILE`) with `{ "token": "<enterprise v3 token>", "captured_at": <epoch ms> }`. With `LM_ARENA_CAPTCHA_MODE=file` the grabber never spawns and the file is the only source.
