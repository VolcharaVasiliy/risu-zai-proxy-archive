...
| Arcee | `trinity-nano-6b`, `trinity-mini`, `trinity-large-preview`, `trinity-large-thinking` | `ARCEE_ACCESS_TOKEN` | `ARCEE_SESSION_ID`, `x-arcee-access-token`, `x-arcee-session-id` | Logged-in `chat.arcee.ai` / `api.arcee.ai` bearer token | `scripts/fetch-arcee-token.mjs`, `scripts/get-arcee-creds.py` | Uses the `api.arcee.ai` `access_token` JWT as a Bearer token (and as the `access_token` refresh cookie). The JWT is **not** in browser cookies/localStorage — it lives in the SPA's memory and is minted by `POST /app/v1/oauth/google` on login. `scripts/fetch-arcee-token.mjs` drives a persistent Edge profile, intercepts that response (body or `Set-Cookie`), and writes the token to `ARCEE_ACCESS_TOKEN` + `arcee_access_token.json`. While the token is still valid, `py/arcee_proxy.py` auto-refreshes via `POST /app/v1/refresh` (returns a fresh 1h token), so active use stays logged in; after a long idle the token expires and you re-run the capture script (the persistent profile usually auto-logs-in via the saved Google session, no code re-entry). Token expires ~60 min (`ACCESS_TOKEN_EXPIRE_MINUTES`). |
