...
| Gemini Web | `gemini-3-flash`, `gemini-3-pro`, `gemini-3-flash-thinking`, plus `gemini-web*` aliases | `GEMINI_WEB_SECURE_1PSID` | `GEMINI_WEB_SECURE_1PSIDTS`, `GEMINI_WEB_COOKIE`, `GEMINI_WEB_MAX_PROMPT_CHARS` | Logged-in `gemini.google.com` / Google cookie session | `scripts/launch-gemini-auth.ps1`, `scripts/get-gemini-web-creds.py`, `scripts/get-provider-creds.py` | Account/region gated. Can auto-use WinINET proxy locally. Long chat history is assembled into one prompt and trimmed from the front at `GEMINI_WEB_MAX_PROMPT_CHARS` to avoid empty candidate frames. |
...
Gemini Web is a browser-session adapter, not the official Gemini API. If a long chat returns `Gemini Web returned no reply candidates`, start a fresh chat or lower `GEMINI_WEB_MAX_PROMPT_CHARS` so the proxy keeps only the most recent dialogue tail.
