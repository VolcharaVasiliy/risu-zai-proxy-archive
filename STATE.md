...
- Fix (primary, infra-free): the proxy auto-resolves `aistudio.xiaomimimo.com` via a **China DoH** (`MIMO_DNS`, default AliDNS `https://223.5.5.5/dns-query`) and pins that China edge IP with curl_cffi `CurlOpt.RESOLVE` — this is exactly what Smart DNS does (the China DNS returns the China A record regardless of where the query originates, so the request lands on the China edge and the guard passes). `MIMO_RESOLVE_IPS` overrides the auto-resolved IP; `MIMO_PROXY` (China egress, fallback to global `HTTPS_PROXY`/`HTTP_PROXY`) is the alternative if the guard is also source-IP based. Both wired into curl_cffi in `py/mimo_proxy.py`. Decode already tries utf-8 → gbk/gb18030 → big5.
- Cannot test locally: from RU the domain is poisoned to `127.0.0.1` by the network/Clash (every DoH resolver returns localhost), and curl_cffi over Clash hits a TLS error (curl:35) — environment limits, not code bugs. The DoH GET/POST + parse path is validated by unit mirror; the real `urlopen` call is confirmed. On Vercel, AliDNS returns the real China IP and the pin works. To verify on Vercel: deploy and check `mimo_dns_resolve` debug log for the China IP, then confirm real Chinese text (not `?????`).
