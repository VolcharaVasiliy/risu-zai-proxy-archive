#!/usr/bin/env python
"""
Extract Mistral console cookies from a dedicated Chromium profile.
Usage: python scripts/get-mistral-creds.py [--profile-root PATH] [--output PATH]
"""
...
def _read_cookie_rows(cookie_db_path: Path):
    ...
    with sqlite3.connect(uri, uri=True) as conn:
        return conn.execute(
            ...
            order by
              case
                when name like 'csrf_token_%' then 0
                when name = '__cf_bm' then 1
                when name = '__cflb' then 2
                else 10
              end,
              host_key,
              name
...
def _extract_cookies(profile_root: Path):
    ...
    copied_db = temp_root / f"{profile_root.name}-mistral-cookies.db"
...
def main():
    ...
    parser.add_argument("--profile-root", default=r"F:\Projects\risu-zai-proxy\auth\mistral-edge-profile")
    ...
    result = {
        ...
        "domains": domains,
        "mistral_cookie": cookie_header,
        "mistral_csrf_token": csrf_token,
    }
