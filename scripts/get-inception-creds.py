#!/usr/bin/env python
"""
Extract Inception browser session cookies from a dedicated Chromium profile.
Usage: python scripts/get-inception-creds.py [--profile-root PATH] [--output PATH]
"""
...
def _read_cookie_rows(cookie_db_path: Path):
    with sqlite3.connect(str(cookie_db_path)) as conn:
        return conn.execute(
            ...
            from cookies
            where host_key like '%chat.inceptionlabs.ai%'
            order by
              case
                when name = 'session' then 0
                else 10
              end,
              host_key,
              name
...
def main():
    ...
    parser.add_argument("--profile-root", default=str(auth_profile("inception", "inception-edge-profile")))
    parser.add_argument("--output", default=str(auth_output("inception", "inception-creds.json")))
    ...
    session_token = str(cookies.get("session") or "")
    ...
    result = {
        ...
        "domains": domains,
        "inception_cookie": cookie_header,
        "inception_session_token": session_token,
    }
