#!/usr/bin/env python
"""
Extract LongCat browser session cookies from a dedicated Chromium profile.
Usage: python scripts/get-longcat-creds.py [--profile-root PATH] [--output PATH]
"""
...
def _read_cookie_rows(cookie_db_path: Path):
    ...
    with sqlite3.connect(uri, uri=True) as conn:
        return conn.execute(
            ...
            from cookies
            where host_key like '%longcat.chat%'
            order by
              case
                when name = 'passport_token_key' then 0
                when name = 'long_cat_region_key' then 1
                else 10
              end,
              host_key,
              name
...
def _extract_cookies(profile_root: Path):
    ...
    copied_db = temp_root / f"{profile_root.name}-longcat-cookies.db"
...
def main():
    ...
    parser.add_argument("--profile-root", default=str(auth_profile("longcat", "longcat-edge-profile")))
    parser.add_argument("--output", default=str(auth_output("longcat", "longcat-creds.json")))
