...
if (-not $ProfileRoot) {
  $ProfileRoot = Get-RzaiAuthProfile -Name 'openaiWeb' -DefaultFolder 'openai-web-edge-profile'
}
...
Write-Output "python scripts\get-openai-web-creds.py --profile-root `"$profileRootResolved`" --output `"$(Get-RzaiAuthOutput -Name 'openaiWeb' -DefaultFile 'openai-web-creds.json')`""
