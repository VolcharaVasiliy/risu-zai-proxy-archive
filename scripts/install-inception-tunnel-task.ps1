...
$action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`" -UpdateVercel -Redeploy"
...
try {
  ...
  @"
@echo off
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$scriptPath" -UpdateVercel -Redeploy
