...
if (-not $ProfileRoot) {
  $ProfileRoot = Get-RzaiAuthProfile -Name 'mistral' -DefaultFolder 'mistral-edge-profile'
}
...
Write-Output "python scripts\get-mistral-creds.py --profile-root `"$profileRootResolved`" --output `"$(Get-RzaiAuthOutput -Name 'mistral' -DefaultFile 'mistral-creds.json')`""
